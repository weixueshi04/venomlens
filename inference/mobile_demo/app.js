...
const state = { bundle: null, health: null, recognitionId: null, lastResult: null, photoMeta: null };
...
async function connect() {
  ...
  try {
    ...
    pill.textContent = live
      ? (state.health.demoReleaseUnverified ? '代理真实·演示lane' : '代理真实')
      : '代理 mock';
...
function updateBanner() {
  ...
  const demoLane = state.health?.demoReleaseUnverified === true;
  $('demo-banner').hidden = !(previewish || demoLane || !state.health);
}
